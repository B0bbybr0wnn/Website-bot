...
async function handleAddPost(chatId, text, env) {
  ...
  if (!activeId) {
    await sendMessage(env.TELEGRAM_TOKEN, chatId,
      "You need an active site first. Use /mysites to pick one, or /newsite to build a site."
    );
    return;
  }
...
async function processNewSiteJob(job, env) {
  ...
  // Replace blog URL placeholder with the actual site's blog URL
  ...
  try {
    ...
    if (body.event === "charge.success") {
      ...
      if (siteId) {
        ...
        const blogUrl = `${baseUrl}/blog/${siteId}`;
        ...
        await sendMessage(env.TELEGRAM_TOKEN, chatId,
          ...
          `• Add blog posts with /addpost\n` +
          `• You have 3 free edits\n\n` +
...
IMPORTANT: Include a navigation link called "Blog" that points to the URL placeholder "BLOG_URL_PLACEHOLDER" — this will be replaced later.
For example: <a href="BLOG_URL_PLACEHOLDER">Blog</a>
Put this Blog link in the navigation menu alongside Home, About, Contact, etc.
