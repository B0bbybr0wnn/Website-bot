...
async function handleUpdate(update, env) {
  try {
    ...
    // ADMIN: /reply command — only works for your chat ID
    if (userText.startsWith("/reply ")) {
      if (String(chatId) !== "6778703420") {
        await sendMessage(env.TELEGRAM_TOKEN, chatId, "Not authorized.");
        return;
      }

      const parts = userText.substring(7).trim().split(" ");
      const targetChatId = parts[0];
      const message = parts.slice(1).join(" ");

      if (!targetChatId || !message) {
        await sendMessage(env.TELEGRAM_TOKEN, chatId,
          `Usage: /reply <chatId> <message>\n\nExample:\n/reply 8508216724 ok bro, working on it`
        );
        return;
      }

      await sendMessage(env.TELEGRAM_TOKEN, targetChatId, message);
      await sendMessage(env.TELEGRAM_TOKEN, chatId, `✅ Sent to ${targetChatId}`);
      return;
    }
...
async function handleDomainSearch(chatId, rawName, env) {
  ...
  if (available.length > 0) {
    for (let i = 0; i < available.length; i += 2) {
      const row = [];
      row.push({ text: `✅ ${available[i].ext}`, callback_data: `order_${name}${available[i].ext}` });
      if (available[i + 1]) {
        row.push({ text: `✅ ${available[i + 1].ext}`, callback_data: `order_${name}${available[i + 1].ext}` });
      }
      buttons.push(row);
    }
  }
